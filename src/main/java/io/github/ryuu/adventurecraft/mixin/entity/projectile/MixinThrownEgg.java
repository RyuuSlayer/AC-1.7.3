    ...
    public void tick() {
        ...
        if (!this.level.isClient) {
            ...
            for (int i1 = 0; i1 < list.size(); ++i1) {
                ...
                if (!entity1.method_1356() || entity1 == this.field_2044 && this.field_2046 < 5 || (movingobjectposition1 = (axisalignedbb = entity1.boundingBox.expand(f4 = 0.3f, f4, f4)).method_89(vec3d, vec3d1)) == null || !((d1 = vec3d.method_1294(movingobjectposition1.field_1988)) < d) && d != 0.0) continue;
                entity = entity1;
                d = d1;
            }
            if (entity != null) {
        ...
        if (movingobjectposition != null) {
            if (movingobjectposition.field_1989 == null || !movingobjectposition.field_1989.damage(this.field_2044, 0)) {
                // empty if block
            }
            if (!this.level.isClient && this.rand.nextInt(8) == 0) {
                int byte0 = 1;
                if (this.rand.nextInt(32) == 0) {
                    byte0 = 4;
                }
                for (int k = 0; k < byte0; ++k) {
                    Chicken entitychicken = new Chicken(this.level);
                    entitychicken.setPositionAndAngles(this.x, this.y, this.z, this.yaw, 0.0f);
                    this.level.spawnEntity(entitychicken);
                }
            }
            for (int j = 0; j < 8; ++j) {
