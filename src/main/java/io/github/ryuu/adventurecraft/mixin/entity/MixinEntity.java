...
public abstract class MixinEntity {
    ...
    public boolean isInsideWall() {
        for (int i = 0; i < 8; ++i) {
            ...
            if (!this.level.canSuffocate(j, k = MathsHelper.floor(this.y + (double)this.getStandingEyeHeight() + (double)f1), l = MathsHelper.floor(this.z + (double)f2)) || !this.level.isFullOpaque(j, k, l)) continue;
            return true;
        }
        return false;
    }
