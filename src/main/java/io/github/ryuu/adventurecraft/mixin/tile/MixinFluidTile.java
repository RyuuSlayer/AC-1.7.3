...
public abstract class MixinFluidTile extends MixinTile {
    ...
    public static double method_1223(TileView iblockaccess, int i, int j, int k, Material material) {
        ...
        if (material == Material.WATER) {
            vec3d = ((MixinFluidTile) Tile.FLOWING_WATER).method_1219(iblockaccess, i, j, k);
        }
        if (material == Material.LAVA) {
            vec3d = ((MixinFluidTile) Tile.FLOWING_LAVA).method_1219(iblockaccess, i, j, k);
        }
     ...
     */
    @Overwrite()
    protected int method_1220(MixinLevel world, int i, int j, int k) {
    ...
    private Vec3f method_1219(TileView iblockaccess, int i, int j, int k) {
        ...
        if (iblockaccess.getTileMeta(i, j, k) >= 8) {
            boolean flag = flag || this.method_1573(iblockaccess, i, j, k - 1, 2);
     ...
     */
    @Overwrite()
    private void method_1222(MixinLevel world, int i, int j, int k) {
        if (world.getTileId(i, j, k) != this.id) {
            return;
        }
        if (this.material == Material.LAVA) {
            boolean flag = flag || world.getMaterial(i, j, k - 1) == Material.WATER;
     ...
     */
    @Overwrite()
    protected void method_1221(MixinLevel world, int i, int j, int k) {
